...
export class PatientViewdetailsComponent implements OnInit {
  ...
  bindToUI() {
    ...
    this.physicianform = new FormGroup({
      ...
      diagnosisdiscription: new FormControl(
        this.postobj.diagnosislist ? this.postobj.diagnosislist : null,
        [Validators.required]
      ),
      ...
      procedurediscription: new FormControl(
        this.postobj.procedureslist ? this.postobj.procedureslist : null,
        [Validators.required]
      ),
      proceduredepricated: new FormControl(null),
      drugid: new FormControl(null),
      drugname: new FormControl(
        this.postobj.drugDescription ? this.postobj.drugDescription : null
      ),
  ...
  ondiscriptionselect(event: any) {
    ...
    this.patient
      .GetidfromDiagnosisdetails(event.value)
      .subscribe((x) => (this.diagnosisidlist = x));
    {
      for (let i = 0; i < this.diagnosisidlist.length; i++) {
        if (event.value === 'others') {
          ...
          this.physicianform.controls['diagnosisid'].setValue(
            this.diagnosisidlist[i].id
          );
        }
      }
  ...
  onselectprocedure(event: any) {
    ...
    for (let i = 0; i < this.listOfprocedure.length; i++) {
      if (event.value === 'others') {
        ...
        this.physicianform.controls['procedureid'].setValue(
          this.listOfprocedure[i].id
        );
      }
    }
